...
<table align="center" border="1" width="70%">
    ...
    <tr>
        <td>Телефоны:</td>
                ...
                <table border="0">
                    <%
                    for (String idPhone : person.getPhones().keySet())
                    {
                    	String phone = person.getPhones().get(idPhone);
                   %>
                    <tr>
                        <td>
                            <%out.write(phone);%>
                        ...
                        <td>
                            <a href="<%=request.getContextPath()%>/?action=editPhone&id=<%=person.getId()%>&idPhone=<%=idPhone%>">Редактировать</a>
                        </td>
                        <td>
                            <a href="<%=request.getContextPath()%>/?action=deletePhone&id=<%=person.getId()%>&idPhone=<%=idPhone%>">Удалить</a>
                        </td>
                    </tr>
                    <%
                        }
